...
const Step1 = ({ brokerAuthData, existingAuthData, onStepChange, setDepositAddress, toAuthData, loading, depositAddress, handleGetDepositAddress, symbol, setSymbol, chain, setChain }) => {
    ...
    return  (
        <div>
          ...
          {!depositAddress ? (
            <GetDepositDetails brokerAuthData={brokerAuthData} depositAddress={depositAddress} toAuthData={toAuthData} onStepChange={onStepChange} symbol={symbol} chain={chain} setChain={setChain} setSymbol={setSymbol} />
          ) : (
    ...
    const Step2 = ({ onStepChange, brokerAuthData, depositAddress, toAuthData, setTransferDetails, handleExecutePreview, loading, formValues }) => {
        console.log('step 2 depositAddress', depositAddress, 'toAuthData: ', toAuthData); 
        return (
          <div>
  ...
  const Step3 = ({ transferDetails, brokerAuthData, depositAddress, handleExecuteTransfer, loading, formValues }) => (
    <div>
      <h2>Submit Transfer</h2>
      {transferDetails ? (
              <ExecuteTransfer brokerAuthData={brokerAuthData} depositAddress={depositAddress} transferDetails={transferDetails} formValues={formValues}/>
            ) : (
              <div>Loading deposit address...</div>
            )}      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2, gap: 2 }}>
  ...
  const TransferModal = ({ open, onClose, brokerAuthData, existingAuthData, onStepChange }) => {
      ...
      const handleExecutePreview = async () => {
        ...
        try {
          ...
          if (!executePreview.ok) {
            ...
            throw new Error(`Failed to Execute Preview Address: ${executePreview.statusText}`);
          }
